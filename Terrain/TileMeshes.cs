...
public static class TileMeshes
{
	...
	private static readonly Vector2[][] _precomputedUVs = new Vector2[TileTypeCount][];
	...
	[MethodImpl(MethodImplOptions.AggressiveInlining)]
	public static Vector2[] GetUVs(TileType type) => _precomputedUVs[(int)type];
	...
	private static void PrecomputeTileData(TileType type)
	{
		...
		_precomputedUVs[(int)type] = CreateUVs(inverted);
	}
	...
	/// <summary>
	/// Per-vertex UV coordinates mapping tile corners to 0..1 quad.
	/// NW=(0,0), NE=(1,0), SE=(1,1), SW=(0,1)
	/// </summary>
	private static Vector2[] CreateUVs(bool inverted)
	{
		Vector2 nw = new(0, 0);
		Vector2 ne = new(1, 0);
		Vector2 se = new(1, 1);
